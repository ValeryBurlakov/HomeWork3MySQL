...
VALUES
 ...
 ('Петр', 'Петров', 'Рабочий', 20, 55000, 60), -- в презентации ему 40 и зп 25000, подрос мужик
 ('Сидр', 'Сидоров', 'Рабочий', 10, 20000, 35), -- 1
 ('Антон', 'Антонов', 'Рабочий', 8, 19000, 28), -- 2
 ('Юрий', 'Юрков', 'Рабочий', 5, 15000, 25), -- 3
 ('Максим', 'Петров', 'Рабочий', 2, 11000, 19),
 ('Юрий', 'Петров', 'Рабочий', 3, 12000, 24), -- 4
 ('Людмила', 'Маркина', 'Уборщик', 10, 10000, 49);
SELECT * FROM staff;

-- TASK 1 Отсортируйте данные по полю заработная плата (salary) в порядке: убывания; возрастания
SELECT 
	id,
	salary,
...
-- TASK 2 Выведите 5 максимальных заработных плат (saraly)
SELECT 
	id,
	salary,
    CONCAT(firstname, " ", lastname) as fullname
...
-- TASK 3 Посчитайте суммарную зарплату (salary) по каждой специальности (роst)
SELECT
	post,
    ROUND(SUM(salary)) AS amount_of_salaries -- целое число
FROM staff
GROUP BY post;

-- TASK 4 Найдите кол-во сотрудников с специальностью (post) «Рабочий» в возрасте от 24 до 49 лет включительно.
SELECT
	COUNT(id),
    post
FROM staff
...
-- TASK 5 Найдите количество специальностей
SELECT
	COUNT(DISTINCT post) as count_posts
FROM staff;

-- TASK 6 Выведите специальности, у которых средний возраст сотрудников меньше 30 лет
-- тут я указал не меньше 30, а меньше либо равно, чтобы хоть что-то выдало !!!
SELECT
	post as "меньше или 30"
FROM staff
GROUP BY post 
HAVING AVG(age) <= 30;
...
SELECT
	COUNT(id) AS count_documents,
	user_id, -- внешний ключ на таблицу users,
	(SELECT email FROM users WHERE users.id = media.user_id) as user_email
FROM media
WHERE media.filename LIKE '%.doc' OR media.filename LIKE '%.docx'
GROUP BY user_id;

-- Посчитать лайки для моих документов (моих медиа)
-- только документы
SELECT 
	likes.user_id as liker, 
	COUNT(*) AS total_likes_for_my_documents
FROM likes
JOIN media ON likes.media_id = media.id
WHERE media.user_id = 9 AND (media.filename LIKE '%.doc' OR media.filename LIKE '%.docx')
GROUP BY likes.user_id;
-- любые медиафайлы
SELECT COUNT(likes.id) AS total_likes_for_my_media
FROM likes 
JOIN media ON likes.media_id = media.id 
WHERE media.user_id = 9; 